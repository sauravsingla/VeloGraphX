...
int main() {
  ...
  // shortcut through a non-existent final edge.
  {
    ...
    b.remove(0,3);
    bfs.apply(b);
    require(bfs.distances()[3] == 3, "final-state: transient add shortened distance(3)");
    require(bfs.distances()[4] == 4, "final-state: transient add shortened distance(4)");
    require(!bfs.last_used_full_recompute(), "final-state: unexpected fallback");
