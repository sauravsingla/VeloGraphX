...
int main() {
  ...
  auto partitions = velographx::plan_numa_vertex_partitions(
      100, info, velographx::NumaMode::auto_detect, 4);
  velographx::WorkStealingPool pool(4);
  velographx::NumaLocalScheduler scheduler(pool, partitions);
  ...
  scheduler.parallel_for_partitions([&](const velographx::NumaVertexPartition& partition) {
    visited.fetch_add(partition.vertex_end - partition.vertex_begin,
