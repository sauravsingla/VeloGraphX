...
referenced result-file hashes. It does not certify scientific validity or turn
CI/synthetic data into a research claim.
"""
...
def validate(payload: dict, manifest_path: Path) -> None:
    ...
    for index, result in enumerate(results):
        ...
        require(isinstance(path_value, str) and path_value, f"results[{index}].path must be non-empty")
        require(isinstance(expected_hash, str) and len(expected_hash) == 64 and
                all(c in "0123456789abcdefABCDEF" for c in expected_hash),
                f"results[{index}].sha256 must be a hexadecimal SHA-256")
        require(isinstance(expected_bytes, int) and expected_bytes >= 0,
                f"results[{index}].bytes must be a non-negative integer")
        ...
        require(existing is not None, f"result file does not exist: {path_value}")
        require(existing.stat().st_size == expected_bytes, f"result byte-size mismatch: {path_value}")
        require(sha256_file(existing).lower() == expected_hash.lower(),
                f"result sha256 mismatch: {path_value}")
